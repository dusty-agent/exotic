export const moodArchive = {

    "aqua-veil": {
  
      slug: "aqua-veil",
  
      title: "Aqua Veil",
  
      category: "Visual Archive",
  
      price: 1490,
  
      count: 6,
  
      sound: true,
  
      description:
        "Dreamlike blue atmosphere inspired by water, flowers and light.",
  
      images: {
  
        cover:
          "/aqua-veil/AQUAVEIL_C_1.png",
  
        gallery: [
  
          "/aqua-veil/AQUAVEIL_C_1.png",
          "/aqua-veil/AQUAVEIL_C_2.png",
          "/aqua-veil/AQUAVEIL_C_3.png",
  
          "/aqua-veil/AQUAVEIL_R_1.png",
          "/aqua-veil/AQUAVEIL_R_2.png",
          "/aqua-veil/AQUAVEIL_R_3.png",
  
        ],

      },
  
    },
  
    "blush-peony": {
  
      slug: "blush-peony",
  
      title: "Blush Peony",
  
      category: "Visual Archive",
  
      price: 1490,
  
      count: 6,
  
      sound: true,
  
      description:
        "Soft pink peonies glowing through dreamy morning light.",
  
      images: {
  
        cover:
          "/blush-peony/BLUSHPEONY_C_1.png",
  
        gallery: [
  
          "/blush-peony/BLUSHPEONY_C_1.png",
          "/blush-peony/BLUSHPEONY_C_2.png",
          "/blush-peony/BLUSHPEONY_C_3.png",
  
          "/blush-peony/BLUSHPEONY_R_1.png",
          "/blush-peony/BLUSHPEONY_R_2.png",
          "/blush-peony/BLUSHPEONY_R_3.png",
  
        ],

      },
  
    },
  
    "greenery-mist": {
  
      slug: "greenery-mist",
  
      title: "Greenery Mist",
  
      category: "Visual Archive",
  
      price: 1490,
  
      count: 6,
  
      sound: true,
  
      description:
        "Quiet forest textures filled with soft green light and mist.",
  
      images: {
  
        cover:
          "/greenery-mist/GREENERYMIST_C_1.png",
  
        gallery: [
  
          "/greenery-mist/GREENERYMIST_C_1.png",
          "/greenery-mist/GREENERYMIST_C_2.png",
          "/greenery-mist/GREENERYMIST_C_3.png",
  
          "/greenery-mist/GREENERYMIST_R_1.png",
          "/greenery-mist/GREENERYMIST_R_2.png",
          "/greenery-mist/GREENERYMIST_R_3.png",
  
        ],

      },
  
    },
  
    "petal-memory": {
  
      slug: "petal-memory",
  
      title: "Petal Memory",
  
      category: "Visual Archive",
  
      price: 1490,
  
      count: 6,
  
      sound: true,
  
      description:
        "A nostalgic collection built from flowers, memories and warm light.",
  
      images: {
  
        cover:
          "/petal-memory/PETALMEMORY_C_1.png",
  
        gallery: [
  
          "/petal-memory/PETALMEMORY_C_1.png",
          "/petal-memory/PETALMEMORY_C_2.png",
          "/petal-memory/PETALMEMORY_C_3.png",
  
          "/petal-memory/PETALMEMORY_R_1.png",
          "/petal-memory/PETALMEMORY_R_2.png",
          "/petal-memory/PETALMEMORY_R_3.png",
  
        ],

      },
  
    },
  
    "white-musk": {
  
      slug: "white-musk",
  
      title: "White Musk",
  
      category: "Visual Archive",
  
      price: 1490,
  
      count: 6,
  
      sound: true,
  
      description:
        "Minimal white atmosphere inspired by soft fabric and spring blossoms.",
  
      images: {
  
        cover:
          "/white-musk/WHITEMUSK_C_1.png",
  
        gallery: [
  
          "/white-musk/WHITEMUSK_C_1.png",
          "/white-musk/WHITEMUSK_C_2.png",
          "/white-musk/WHITEMUSK_C_3.png",
  
          "/white-musk/WHITEMUSK_R_1.png",
          "/white-musk/WHITEMUSK_R_2.png",
          "/white-musk/WHITEMUSK_R_3.png",
  
        ],
        
      },
  
    },
  
    "original-collection": {
  
      slug: "original-collection",
  
      title: "Original Collection",
  
      category: "Visual Archive",
  
      price: 1490,
  
      count: 3,
  
      sound: true,
  
      description:
        "Original dreamscapes created as the foundation of Mood Archive.",
  
      images: {
  
        cover:
          "/original-collection/ORIGINALCOLLECTION_C_1.png",
  
        gallery: [
  
          "/original-collection/ORIGINALCOLLECTION_C_1.png",
          "/original-collection/ORIGINALCOLLECTION_C_2.png",
          "/original-collection/ORIGINALCOLLECTION_C_3.png",
          "/original-collection/ORIGINALCOLLECTION_A_1.png",
          "/original-collection/ORIGINALCOLLECTION_A_2.png",
          "/original-collection/ORIGINALCOLLECTION_A_3.png",
  
        ],

      },
  
    },
  
  };